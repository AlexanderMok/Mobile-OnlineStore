...
<html>
  ...
  <body>
    ...
    <tr>
          <td width="37%">期望发货时间：</td>
          <td width="63%"> ${order.uszdday}${order.uszdtime}</td>
    </tr>
  	<tr>
          <td width="37%">订单状态：</td>
    ...
    <c:if test="${order.state eq '交易完成' or order.state eq '配送中'}">
    	<tr>
   ...
   <c:choose>
    ...
    <tr>
 	...
 	<br/>
 		...
 		<tr align="center"  height="50">
 			...
 			<td>${count}</td>
 			<td>${subtotal}</td>
 			<td>${tblItems.tblSuppliers.suppName}</td>
 		</tr>
 		</s:iterator>
 		</c:when>
 		<c:otherwise>
 		...
 		<tr align="center" height="50">
 			...
 			<td>${order.pinId.tblItems.tblSuppliers.suppName}</td>
 			<td>${order.pinId.startTime}</td>
 			<td>${order.pinId.endTime}</td>
    ...
    <center>  
    <c:choose>
    <c:when test="${order.pinId.totalCount <= order.pinId.nowCount}">
	    <form action="pages/back/storekeeper-updateorderstate?id=${oid}&state=5" method="post">
	 	请选择送货人：<select name="senderid">
	 	<s:iterator value="senderlist">
	 	<option value="${id}">${psrname}</option>
	 	</s:iterator>
	 	</select>
	 	<input type="submit" value="发货"  onclick="javascript:return confirm('您确定已发货了吗？')"/>
    </c:when>
    <c:otherwise><font color="red" size="4">拼单人数不足，不能发货！</font></c:otherwise>
    </c:choose>
