...
<SCRIPT type="text/javascript">
    ...
    function tijiao(){
    	...
    	phone.value = b;
    	//收货人名
    	var shouhuoname = document.getElementById("shouhuoname");
    	var lianxi = document.getElementById("lianxi").innerText;
    	shouhuoname.value = lianxi;
    	//判断购物车是否为空
    	var gwc = document.getElementById("priceiId").innerText;
    	var psd = document.getElementById("psday");
    	if(gwc == "0.0"){
    		alert("亲，你的购物车空空如也");
    	}else if(psd.value == ""){
    		alert("请输入配送日期");
    	}else{
    		alert(shouhuoname.value);
...
<body>
		...
		<form name="bookForm" id="myform" action="pages/front/toOrder.action"
					method="post">
		<c:forEach var="sl" items="${slist}">
		<div class="shangjia_content" id="seller${sl.shopcarId}">
			<div class="content">
			<div class="lanmu_top">
				<div class="spm">
					${sl.tblItems.itemName}
				</div>
				<div class="liebiao">
					<ul>
					
										<li><img alt="" src="source/images/jia.png" class="add" id="add${sl.user.userid}"></li>
										<li><input readonly="readonly" class="count-input hide count-inputr${sl.shopcarId}" type="text" name="num" value="1" id="count-input${sl.user.userid}"/></li>
										<li><img alt="" id="reduce${sl.user.userid}" src="source/images/jian.png" class="reduce hide"></li>
					</ul>
				</div>
				
			</div>
			<div class="lanmu_down">
				<div class="jiage">¥<span>${sl.tblItems.retailPrice}</span>
				</div>
				<div class="lajitong">
					<a href="pages/front/delShopcar?sid=${sl.shopcarId}"><img alt="" src="source/images/lajitong.png" class="delete" ></a>
				</div>
			</div>
		...
		<div class="zongshu_1">
			<p>
				数量：<span id="numId">1</span>份 应付金额：<span id="priceiId" class="qqian"></span>元
			</p>
		</div>
	</div>
	
	<div class="kj_right" onclick="javascript:window.location.href='pages/front/user_myAddress.action?user.userid=${user.userid}&flag=1'">
     <div class="shang_wenzi"><div id="lianxi" class="lianxi">
  ...
  <div class="xia_wenzi" id="yemiandizhi">配送地址：<%if(session.getAttribute("address") == null){ %>${sessionScope.user.defaultaddress}<%}else { %>${sessionScope.address.addr}<%} %></div>
   <img class="black_jiantou" alt="" src="source/images/hyjt.png">
          ...
          <div style="background-color: #fff; width:95%;margin: 110px auto -40px auto;">
					...
					<input type="hidden" id="usedbalanceType" name="usedbalanceType" value="0" /> 
					<input type="hidden"/>
					<div id="numi"></div>
					<input type="hidden" id="shouhuoname" name="receivername" />
					...
					<div class="qb_mb10 qb_flexx"style="width: 95%; margin: 5px auto;">
						<span class="mod_select qb_mr10 qb_mb10" id="sprovince1"> 
								<input type="Date" id="psday" name="psday" style="height: 39px; color: #999999"
							class="mod_input flex_box"/>
					...
					<div class="qb_mb10 qb_flexx" style="width: 95%; margin: 5px auto;">
						<span class="mod_select qb_mr10 qb_mb10" id="sprovince"> <select
							name="pstime" id="pstime" style="height: 39px; color: #999999"
							class="mod_input flex_box">
							
								<option value="0" selected="selected">配送时间：下午  13:00~16:00</option>
								<option value="1">立即配送 （需另加配送费6元）</option>
						</select>
						</span>
					</div>
					...
					<div class="qb_mb10 qb_flexx"style="width: 95%; margin: 5px auto;">
						...
						<select id="pstype" name="paystate" style="height: 39px; color: #999999"
